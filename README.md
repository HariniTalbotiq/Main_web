...
`docs/seo/` is the strategy's deliverable set and the place to start. In one paragraph:

- **`tools/seo-pass.js`** runs over the 24 hand-written pages the way `tools/clean-urls.js`
  does — idempotent, zero dependencies, safe on every build. It self-hosts Inter (the Google
  Fonts stylesheet was 1.3–2.0 s of every page's mobile LCP), extracts the two base64 logos to
  `assets/brand/` (~100 kB of HTML per page), sets `width`/`height` on every image, adds the
  breadcrumb the solution pages already had to the product pages, applies titles and
  descriptions from **`seo/meta.json`** (≤60 / ≤155, enforced), inserts an FAQ block from
  **`seo/faq/<slug>.json`** in the page's own accordion markup with `FAQPage` schema, dates
  the page from git, and makes the demo videos load on scroll instead of on parse.
- **`/products` and `/solutions`** are generated by `build.js` from **`seo/hub.json`** (copy)
  and **`seo/formats.json`** (the comparison table, every cell from the page it links to).
  Both were 404s that every product and solution page's breadcrumb schema already named.
- **`scripts/generate-sitemap.mjs`** now writes `lastmod` from git and `llms.txt` from the same
  walk; **`scripts/indexnow.mjs`** pings Bing after a deploy, once the domain is live.

Order when a mockup is re-dropped: `fix-pages.js` → `clean-urls.js` → `npm run site`
(which is `seo-pass` → `build` → `sitemap`). Edit the JSON, never the generated blocks.

What was deliberately **not** built — a pricing page, competitor comparisons, an "affordable"
page — and what unblocks each is in `docs/seo/01-page-map.md` and `02-content-briefs.md`.
None of it can be written from this repo: the facts (prices, competitors, profile URLs) are yours.
...
- **There is no pricing page,** and the `Pricing` nav item is gone rather than
  pointed at the contact form (see `home.js`). The SEO strategy makes a public
  price list the single highest-leverage page on the site; the brief for it,
  and the exact figures it needs, is `docs/seo/02-content-briefs.md`.
